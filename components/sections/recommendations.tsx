...
export function RecommendationsSection() {
  ...
  return (
    <section className="snap-section section-dark py-24 overflow-hidden" id="recommendations">
      ...
      {/* Marquee: pauses on hover, cards pop up */}
      <div className="relative flex overflow-x-hidden group/marquee">
        {/* First copy */}
        <div className="animate-marquee group-hover/marquee:[animation-play-state:paused] flex gap-8 shrink-0 px-4">
          {recommendations.map((rec, index) => (
            <div
              key={`a-${index}`}
              className="w-[350px] md:w-[460px] shrink-0 p-8 pt-10 rounded-2xl flex flex-col justify-between border transition-all duration-300 hover:scale-105 hover:-translate-y-2 hover:shadow-2xl hover:z-10 cursor-default"
              style={{
            ...
            >
              <div className="mb-8">
                ...
                <p className="leading-relaxed font-medium text-base md:text-lg" style={{ color: "hsl(0 18% 93% / 0.85)" }}>
                  &ldquo;{rec.text.length > 250 ? rec.text.slice(0, 250) + "..." : rec.text}&rdquo;
                </p>
              </div>
        ...
        <div className="animate-marquee group-hover/marquee:[animation-play-state:paused] flex gap-8 shrink-0 px-4" aria-hidden="true">
          {recommendations.map((rec, index) => (
            <div
              key={`b-${index}`}
              className="w-[350px] md:w-[460px] shrink-0 p-8 pt-10 rounded-2xl flex flex-col justify-between border transition-all duration-300 hover:scale-105 hover:-translate-y-2 hover:shadow-2xl hover:z-10 cursor-default"
